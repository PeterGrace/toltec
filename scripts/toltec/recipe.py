...
{pkg_name}() for corresponding package"
                ...
                self.packages[pkg_name] = Package(pkg_name, self, functions[pkg_name])
    ...
    def make(
        ...
        for package in packages if packages is not None else self.packages.keys():
            if package not in self.packages:
                raise BuildError(
                    f"Package {package} does not exist in \
recipe {self.name}"
                )
...
source file '{source}', got {req.status_code}"
    ...
    def build(self, src_dir: str, docker: DockerClient) -> None:
        ...
        logs = bash.run_script_in_container(
            ...
            mounts=[
                Mount(type="bind", source=os.path.abspath(src_dir), target=mount_src)
            ],
...
class PackageAdapter(logging.LoggerAdapter):
    ...
    def process(self, msg, kwargs):
        return (
            "%s (%s): %s" % (self.extra["package"], self.extra["recipe"], msg),
            kwargs,
        )
...
for package {self.name}"
        ...
        self.logger = PackageAdapter(
            logger, {"recipe": parent.name, "package": self.pkgid()}
        )
...
"""
    ...
    def package(self, src_dir: str, pkg_dir: str) -> None:
        ...
        logs = bash.run_script(
            script=self.action,
            variables={**self._bash_variables, "srcdir": src_dir, "pkgdir": pkg_dir},
        )
    ...
    def archive(self, pkg_dir: str, ar_dir: str) -> None:
        ...
        scripts = {}
        variables: bash.Variables = {**self._bash_variables, "pkgname": self.name}
        script_header = f"""\
#!/usr/bin/env bash
set -e
...
def _check_field_indexed(
    variables: bash.Variables, name: str, default: Optional[bash.IndexedArray] = None
) -> bash.IndexedArray:
    if name not in variables:
        if default is None:
            raise InvalidRecipeError(f"Missing required field '{name}'")
